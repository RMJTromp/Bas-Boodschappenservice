<?php
    ...
    use Boodschappenservice\exceptions\JSONException;
    ...
    class JSON {
         ...
         * @throws JSONException Thrown if any errors occurs when decoding the json string
         */
        public static function decode(string|File $json, ?bool $associative = false, int $depth = 512, int $flags = 0) : mixed {
            $res = json_decode($json instanceof File ? ($json->getContents() ?? "{}") : $json, $associative, $depth, $flags);
            if(json_last_error() !== 0) throw new JSONException(json_last_error_msg(), json_last_error());
            return $res;
        }
        ...
        #[NoReturn]
        public static function printAndExit(mixed $object, int $flags = 0, int $depth = 512) : void {
            printAndExit($object === "{}" ? "{}" : JSON::encode($object, $flags, $depth), "application/json; charset=UTF-8");
        }

        public static function isJSON(string $string) : false|array {
