<?php
    ...
    class File implements \JsonSerializable {
        ...
        private string $path;
        private static ?MimeTypes $mimes = null;
         ...
         * failure.
         */
        public static function createTempFile(string $directory = null, string $prefix = null) : File|false {
         ...
         * </p>
         */
        public static function getWorkingDirectory() : File {
            if(self::$cwd === null) self::$cwd = new File(getcwd());
            return self::$cwd;
        }
        ...
        /**
         * Returns directory used for temporary files
         * @link https://php.net/manual/en/function.sys-get-temp-dir.php
         * @return File the file instance of the system's temporary directory.
         * @since 5.2.1
         */
        public static function getTempDirectory() : File {
            if(File::$tempDir === null) File::$tempDir = new File(sys_get_temp_dir());
            return File::$tempDir;
        }
        ...
        public function delete($context = null) : bool {
            if($this->isFile()) return unlink($this->path, $context);
            else if($this->isDirectory()) {
                // delete all contents of the file before deleting folder (causes error if not)
                $files = $this->listFiles();
                foreach ($files as $file) $file->delete();

                return rmdir($this->path, $context);
            }
            return false;
        }
         ...
         * the manual.
         * </p>
         * @return File[] an array of filenames on success, or false on
         * failure. If directory is not a directory, then
         * boolean false is returned, and an error of level
         * E_WARNING is generated.
         */
        public function listFiles(int $sorting_order = 0, $context = null) : array {
            $files = scandir($this->path, $sorting_order, $context);
            return array_map(function($item) {
                return new File($this->path . "/" . $item);
         ...
         * @return bool true on success or false on failure.
         */
        public function renameTo(string $to, $context = null) : bool {
            return rename($this->path, Path::resolve($to), $context);
        }
         ...
         * @return bool true on success or false on failure.
         */
        public function copyTo(string|File $to, $context = null) : bool {
            return copy($this->path, $to instanceof File ?  $to->path : $to, $context);
        }
        ...
        public function getMimeType() : string|false  {
            if($this->getExtension() === "mjs") return "application/javascript";
            if(self::$mimes === null) self::$mimes = new MimeTypes();
            return self::$mimes->getMimeType($this->getExtension()) ?? mime_content_type($this->getPath());
        }
         ...
         * @link https://php.net/manual/en/function.pathinfo.php
         * @return File|null file parent directory name
         */
        public function getDirectory() : ?File {
            $dirName = $this->getDirectoryName();
            return $dirName !== false ? new File($dirName) : null;
        }
